...
estimate_mysql_db_size() {
    ...
    "$mysql_cmd" --defaults-extra-file="$defaults_file" --batch --skip-column-names -e \
...
check_mysql_access() {
    ...
    local mysql_cmd
    mysql_cmd="$(mysql_client_cmd)"
    if ! "$mysql_cmd" --defaults-extra-file="$defaults_file" --batch --skip-column-names -e "SELECT 1;" "$DB_NAME" >/dev/null 2>&1; then
        rm -f "$defaults_file"
...
dump_mysql_database() {
    ...
    estimated_size="$(estimate_mysql_db_size "$defaults_file")"
    ...
    if command_exists pv && [ "$estimated_size" -gt 0 ] && [ "${PROGRESS_MODE:-auto}" != "off" ]; then
        "$dump_cmd" --defaults-extra-file="$defaults_file" \
